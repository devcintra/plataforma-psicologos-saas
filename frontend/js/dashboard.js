...
async function carregarDashboard() {
    ...
    try {
        const resposta = await fetch(`${API_BASE_URL}/psicologos/${id}`,
            {
...
function preencherPerfil(psicologo) {
    ...
    const nome =musuario.nome || "Profissional";
    ...
    const foto = psicologo.foto || "htps://images.unsplash.com/photo-1535713875002-d1d0cf377fde?q=80&w=150";
    ...
    const nomeCampo = document.getElementById("dash-nome");

    if (nomeCampo) {
        nomeCampo.textContent = nome;
    }

    const boasVindas =document.getElementById("nav-welcome");
    if (boasVindas) {
        boasVindas.textContent =`Olá, ${nome.split(" ")[0]}`;
    }
    const crp =document.getElementById("dash-crp");

    if (crp) {
        crp.textContent =`CRP ${psicologo.crp}`;
    }

    const abordagem =document.getElementById("dash-abordagem");

    if (abordagem) {
        abordagem.textContent = especialidade;
    }

    const containerEspecialidades = document.getElementById("spec-tags");
...
function configurarNavegacaoAbas() {
    const btnInicio =document.getElementById("menu-btn-inicio");
    const btnConsultas =document.getElementById("menu-btn-consultas");
...
function renderizarHorarios(disponibilidades) {
    ...
    dias.forEach(dia => {
        ...
        if (horariosDoDia.length === 0) {
            conteudoHoras = `<span class="no-hours">Nenhum horário</span>`;
        } else {
        ...
        coluna.innerHTML = 
            `<div class="day-name">${nomeDia.charAt(0).toUpperCase() + nomeDia.slice(1)} </div>
            <div class="hours-list">${conteudoHoras}</div>`;
...
window.openModal = async function () {
    const dia = prompt("Digite o dia (segunda, terca, quarta, quinta, sexta, sabado ou domingo):");
    ...
    const horaInicio = prompt("Digite o horário inicial (Ex: 08:00):");
    if (!horaInicio) return;

    const horaFim = prompt("Digite o horário final (Ex: 09:00):");
    if (!horaFim) return;
    ...
    try {
        ...
        if (!resposta.ok) {
            throw new Error(resultado.erro || "Erro ao cadastrar horário");
        }
        ...
        console.error(erro);
        alert(erro.message || "Erro ao adicionar horário.");
    }
};
...
async function carregarConsultas() {
    ...
    try {
        ...
        if (!consultas.length) {
            ...
            container.innerHTML = `
                <div style="text-align:center;padding:40px;color:var(--plum);">
    ...
    <div class="c-info">
        ...
        <div class="c-detalhes">
            <span> 🗓️ ${data}</span>
            <span style="margin-left:15px;">🕒 ${hora}</span> 
            <spanstyle="margin-left:15px;color:green;font-weight:600;">● ${status}</span>
        </div>
    </div>

    <div class="c-actions">
        <a href="chat.html?tipo=psicologo&id=${consulta.id_paciente}"class="btn-action-chat">💬 Abrir Chat</a>
        <a href="https://meet.google.com/" target="_blank" class="btn-action-meet" style="  margin-left:10px;">🎥 Google Meet</a>
        </div>`;
...
function configurarNotificacoes() {
    ...
    btn.addEventListener("click", () => {
        alert("Você não possui notificações no momento.");
    });
}